...
- 구현·콘텐츠 작성은 **서브에이전트(Agent 도구, model: sonnet — 큰 설계 변경은 opus)에 위임**,
  메인(Fable)은 지시·리뷰·테스트·커밋만. 1~2줄 수정만 직접. 이유: Fable 출력 토큰이 비싸
  주간 한도를 빨리 소진함 (codex 위임 폐지 후 대체 — 2026-08-03).
  agy(Gemini)는 이 프로젝트에서 사용 금지.
