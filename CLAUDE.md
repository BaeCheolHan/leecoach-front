...
- 파일 단위 구현·콘텐츠 작성은 codex(mcp__codex__codex)에 위임, Claude는 지시·리뷰·테스트·커밋.
  1~2줄 수정만 직접. agy(Gemini)는 이 프로젝트에서 사용 금지.
- 커밋·push(=배포)는 사용자가 명시적으로 요청한 뒤에만. 배포 후 운영 URL curl 확인까지.
- 검증은 codex 자기 보고를 믿지 않고 host에서 직접 실행. 기존 테스트 약화 여부를 diff로 확인.
